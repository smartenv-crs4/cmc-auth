...
describe('UserAndAppTypes Auth Model', function(){

  before(function(done){

    db.connect("userAndAppTypes Model",function(){
  ...
  describe('findAll({skip:2, limit:30})', function(){
    ...
    it('must include _metadata with correct values', function(done){
      ...
      UserAndAppTypes.findAll({}, null, {skip:2, limit:30}, function(err, results){
          if(err) throw err;
          else{
            results.should.have.property('_metadata');
            results.userandapptypes.length.should.be.equal(30);
  ...
  describe('findAll({skip:0, limit:10})', function(){
    ...
    it('must include _metadata with correct values', function(done){
      UserAndAppTypes.findAll({}, null, {skip:0, limit:10}, function(err, results){
          if(err) throw err;
          else{
            results.should.have.property('_metadata');
            results.userandapptypes.length.should.be.equal(10);
  ...
  describe('findAll() no pagination', function(){
    ...
    it('must include _metadata with default values', function(done){
      ...
      UserAndAppTypes.findAll({}, null, null, function(err, results){

          if(err) throw err;
          else{
            results.should.have.property('_metadata');
            results.userandapptypes.length.should.be.equal(50);
            results._metadata.skip.should.be.equal(0);
  ...
  describe('findAll({skip:0, limit:2})', function(){
    ...
    it('must include _metadata with correct values and only 2 entries', function(done){
      ...
      UserAndAppTypes.findAll({}, null, {skip:0, limit:2}, function(err, results){

          if(err) throw err;
          else{
            results.should.have.property('_metadata');
            results.userandapptypes.length.should.be.equal(2);
            results._metadata.skip.should.be.equal(0);
  ...
  describe('findOne()', function(){
    ...
    it('must include all required properties', function(done){
      ...
      UserAndAppTypes.findOne({}, null, function(err, user){
